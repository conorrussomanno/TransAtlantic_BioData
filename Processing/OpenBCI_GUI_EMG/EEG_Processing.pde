...
class EEG_Processing_User {
  ...
  int netBridge_output = 0; //value between 0-100 that is the relative position of the current uV average between the rolling lower and upper uV thresholds
  float netBridge_output_normalized = 0;
  float netBridge_output_adjusted = 0;
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    // netBridge EMG output code ... to be sent over Arduino on Serial/COM port 2
...
//    }  
    ...
    netBridge_output = (int)map(myAverage, lowerThreshold, upperThreshold, 0, 100);
    netBridge_output_normalized = map(myAverage, lowerThreshold, upperThreshold, 0, 1);
    netBridge_output_adjusted = ((-0.1/(netBridge_output_normalized*100.0)) + 100.0);
    
    if(netBridge_output < 0) netBridge_output = 0;
    if(netBridge_output > 100) netBridge_output = 100;
 
//    println("netBridge_output: | " + (int)netBridge_output_adjusted + " |");
//    netBridge_serial.write((int)netBridge_output_adjusted); 
    
    println("netBridge_output: | " + netBridge_output + " |");
    if(netBridge_output < 75){
      //write off
//      c.write("off" + "\n");
      isActive = false;
    } else {
...
//      c.write("on" + "\n");
  ...
  public void draw(){
    pushStyle();
      ...
      rect(7*(width/8), 3*(height/8), (width/32), map(netBridge_output_normalized, 0, 1, 0, (-1) * (height/4)));
